-- GalaxyHub standalone UI test #5
...
status.Text = "UI OK\nĐang test: GalaxyHub_MagnetToken"
...
local source = [===[--[[ GalaxyHub | BloxFruit | FILE 4: Magnet Token (Summer Token - Banana logic) ]]
local Players = game:GetService("Players")
...
-- Electrified mob detection (Summer Token mobs có hiệu ứng điện)
local function isElectrified(mob)
    for _, child in pairs(mob:GetDescendants()) do
        ...
        if child:IsA("Attachment") and child.Name:lower():find("electric") then
            return true
        end
    end
    return false
...
local function getNearestElectrified()
...
function Magnet.start()
    ...
    task.spawn(function()
        while Magnet.Enabled do
            pcall(function()
                local target = getNearestElectrified()
                if target then
                    teleportTo(target)
                    attack()
                end
...
if ok then
    status.Text = "UI OK\nCODE OK • GalaxyHub_MagnetToken"
    status.TextColor3 = Color3.fromRGB(120, 255, 160)
else
    status.Text = "UI OK\nCODE ERROR\n" .. tostring(err):sub(1, 120)
    status.TextColor3 = Color3.fromRGB(255, 120, 120)
    warn("[GalaxyHub TEST #5] GalaxyHub_MagnetToken:", err)
end
