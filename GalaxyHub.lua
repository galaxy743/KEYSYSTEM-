-- ONAX • Blox Fruits | Key System
-- Đồng hồ góc phải dựa trên thời điểm hết hạn từ máy chủ.
...
local function ShowSuccess(data)
    ...
    I.Text = "Thời hạn: "..tostring(data.days or "?").." ngày"
    ...
    -- Góc phải: đồng hồ còn lại.
    local Timer = Instance.new("TextLabel",Gui)
    Timer.Name = "ONAX_KeyTimer"
    Timer.AnchorPoint = Vector2.new(1,0)
    Timer.Position = UDim2.new(1,-18,0,18)
    Timer.Size = UDim2.new(0,190,0,38)
    Timer.BackgroundColor3 = Color3.fromRGB(8,20,38)
    Timer.BackgroundTransparency = 0.08
    Timer.BorderSizePixel = 0
    Timer.TextColor3 = Color3.fromRGB(80,255,140)
    Timer.TextSize = 14
    Timer.Font = Enum.Font.GothamBold
    Timer.Text = "KEY: đang tải..."
    corner(Timer,10)
    stroke(Timer,Color3.fromRGB(0,200,120))

    -- expires là Unix timestamp do máy chủ trả về.
    -- Vì vậy thoát game không làm thời gian dừng lại.
    ...
    if not expires then
        Timer.Text = "KEY: không xác định"
    else
        task.spawn(function()
            while Gui.Parent and Timer.Parent do
                local remaining = expires - os.time()

                if remaining <= 0 then
                    Timer.Text = "KEY: HẾT HẠN"
                    Timer.TextColor3 = Color3.fromRGB(255,90,90)
                    I.Text = "Key đã hết hạn"
                ...
                Timer.Text = "KEY: "..formatRemaining(remaining)
                I.Text = "Còn lại: "..formatRemaining(remaining)
                task.wait(1)
            end
        end)
    end
    ...
    Continue.MouseButton1Click:Connect(function()
        -- Thay URL_SCRIPT_CHINH bằng raw URL script chính của bạn.
...
Verify.MouseButton1Click:Connect(function()
    ...
    if not decoded or type(data) ~= "table" then
        Verify.Text = "✓  XÁC NHẬN"
        Status.Text = "● Dữ liệu không hợp lệ."
        return
    end
