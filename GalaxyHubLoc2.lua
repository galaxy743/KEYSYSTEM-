-- GalaxyHubLoc.lua | merged build
-- Built from the available GalaxyHub/Banana/ONAX source files.
-- Module order: environment -> ONAX -> shared logic -> UI -> features -> loops.

-- =========================================================
-- ONAX KEY SYSTEM
-- Nhap key duoc tao tu: https://onax.onrender.com/
-- Key duoc kiem tra server; lan dau su dung moi bat dau tinh han.
-- =========================================================

local ONAX_URL = "https://onax.onrender.com/api/check_key"
local ONAX_KEY_GUI = "ONAX_Key_Login"
local ONAX_REQUEST = (syn and syn.request) or (http and http.request) or http_request or request

local function onaxCreateLogin()
    local CoreGui = game:GetService("CoreGui")
local Players = game:GetService("Players")

    pcall(function()
        local old = CoreGui:FindFirstChild(ONAX_KEY_GUI)
        if old then old:Destroy() end
    end)

    local gui = Instance.new("ScreenGui")
    gui.Name = ONAX_KEY_GUI
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.DisplayOrder = 999999

    -- Android executor compatibility: try gethui -> CoreGui -> PlayerGui.
    ...
    if not parented then
        warn("[GalaxyHub] Không thể tạo ScreenGui trên executor này.")
        return nil
    end

    local frame = Instance.new("Frame")
    frame.Size = UDim2.fromOffset(360, 215)
    ...
    box.Position = UDim2.fromOffset(15, 76)
    box.BackgroundColor3 = Color3.fromRGB(8, 8, 8)
    box.BorderSizePixel = 0
    box.PlaceholderText = "ONAX-XXXXXXXX..."
    ...
    button.Position = UDim2.fromOffset(15, 124)
    ...
    status.Position = UDim2.fromOffset(15, 164)
    status.BackgroundTransparency = 1
    status.Text = "[1/3] GalaxyHub đã khởi động"
    ...
    timeLabel.Size = UDim2.new(1, -30, 0, 22)
    timeLabel.Position = UDim2.fromOffset(15, 188)
    ...
    return gui, box, button, status, timeLabel
end

local function onaxCheckKey(key)
    ...
    local ok, body = pcall(function()
        local url = ONAX_URL .. "?key=" .. game:GetService("HttpService"):UrlEncode(key)
        if ONAX_REQUEST then
            local r = ONAX_REQUEST({Url = url, Method = "GET"})
    ...
    if not ok then
        ONAX_STATUS.Text = "[2/3] Lỗi HTTP: " .. tostring(body)
        return false, "Không kết nối được ONAX Server."
    end
    if type(body) ~= "string" or body == "" then
        ONAX_STATUS.Text = "[2/3] ONAX không trả dữ liệu"
        return false, "ONAX Server không trả dữ liệu."
    end
    ...
    if not decodedOk or type(data) ~= "table" then
        return false, "Server trả dữ liệu không hợp lệ."
    end

    if data.valid == true or data.success == true or data.authorized == true or data.status == "valid" then
        ONAX_STATUS.Text = "[3/3] ONAX xác thực thành công"
        return true, data.first_use and "Key hợp lệ • Đã kích hoạt" or "Key hợp lệ", data.expires
    end

    return false, data.error or "Key không hợp lệ hoặc đã hết hạn.", nil
end

local ONAX_GUI, ONAX_BOX, ONAX_BUTTON, ONAX_STATUS, ONAX_TIME = onaxCreateLogin()
if not ONAX_GUI then
    warn("[GalaxyHub] STOP: không tạo được Key UI.")
    return
end

-- Mobile debug: luôn báo trạng thái để không còn trường hợp chạy im lặng.
pcall(function()
    ONAX_STATUS.Text = "[1/3] UI Key đã tạo • chờ nhập Key"
end)
local ONAX_UNLOCKED = false

local function onaxTry()
    if ONAX_UNLOCKED then return end

    ONAX_BUTTON.Text = "ĐANG KIỂM TRA..."
    ONAX_STATUS.Text = ""
    ONAX_BUTTON.Active = false

    local valid, message, expires = onaxCheckKey(ONAX_BOX.Text)

    if valid then
        ONAX_UNLOCKED = true
        ONAX_STATUS.TextColor3 = Color3.fromRGB(80, 255, 150)
        ONAX_STATUS.Text = message
        ...
        if expires then
            ...
            ONAX_TIME.Text = string.format("Còn lại: %d ngày %02d giờ %02d phút", days, hours, mins)
            ONAX_TIME.TextColor3 = Color3.fromRGB(180, 220, 255)
        end

        ONAX_BUTTON.Text = "THÀNH CÔNG"
        task.wait(1.5)
        ONAX_GUI:Destroy()
    else
        ONAX_STATUS.TextColor3 = Color3.fromRGB(255, 90, 90)
        ONAX_STATUS.Text = message
        ONAX_BUTTON.Text = "KIỂM TRA KEY"
        ONAX_BUTTON.Active = true
    end
end

ONAX_BUTTON.MouseButton1Click:Connect(onaxTry)
ONAX_BOX.FocusLost:Connect(function(enterPressed)
    if enterPressed then
        onaxTry()
    end
end)

-- Chặn toàn bộ phần script chính cho tới khi key hợp lệ.
repeat
    task.wait()
until ONAX_UNLOCKED
...
-- Executor/loader compatibility: these hooks are optional.
-- If an executor does not support hookfunction or the module path changes,
-- do not stop the whole hub from loading.
pcall(function()
...
pcall(function()
    if type(hookfunction) == "function" then
        local Respawn = require(game:GetService("ReplicatedStorage").Effect.Container.Respawn)
...
local _redzSourceOk, _redzSource = pcall(function()
    return game:HttpGet("https://raw.githubusercontent.com/PlockScripts/Library-ui/refs/heads/main/redz-V5-remake/main.luau")
end)
if not _redzSourceOk or type(_redzSource) ~= "string" or _redzSource == "" then
    warn("[GalaxyHub] Không tải được Redz UI library.")
    return
end

local _redzLoadOk, vu32 = pcall(function()
    return assert(loadstring(_redzSource))()
end)
if not _redzLoadOk or type(vu32) ~= "table" then
    warn("[GalaxyHub] Redz UI library load lỗi: " .. tostring(vu32))
    return
end

local _windowOk, v466 = pcall(function()
    return vu32:MakeWindow({
        Title = "GalaxyHub | BloxFruit",
        SubTitle = "ONAX • GalaxyHub",
        SaveFolder = "GalaxyHub | ONAX.lua"
    })
end)
if not _windowOk or not v466 then
    warn("[GalaxyHub] Không tạo được cửa sổ UI: " .. tostring(v466))
    return
end
...
-- =========================================================
-- GALAXYHUB LOC PATCH: BANANA SUMMER / MAGNET TOKEN
-- Giữ nguyên logic Banana: target mob [Electrified] theo từng Sea.
-- =========================================================
_G.FarmSummer = false

local function GalaxyHubLoc_GetSummerPos()
...
v485:AddToggle({
    Name = "Auto Farm Magnet Tokens",
    Description = "Farm Summer Token bằng mob [Electrified] theo logic Banana",
    Default = false,
    Callback = function(Value)
        _G.FarmSummer = Value
        if not Value then
            pcall(function() StartBring = false end)
        end
    end
})
...
task.spawn(function()
    while task.wait() do
        if _G.FarmSummer and not _G.AutoFarm then
            pcall(function()
                local summerPos = GalaxyHubLoc_GetSummerPos()
                if not summerPos then return end
                ...
                for _, mob in pairs(workspace.Enemies:GetChildren()) do
                    ...
                    if hum and root and hum.Health > 0 and hum.DisplayName:find("%[Electrified%]") then
                        target = mob
                        break
                    end
                end
                ...
                if not target then
                    if (hrp.Position - summerPos.Position).Magnitude > 50 and type(TweenTo) == "function" then
                        TweenTo(summerPos)
                    end
                    return
                end
                ...
                if type(AutoHaki) == "function" then pcall(AutoHaki) end
                if type(EquipWeapon) == "function" then pcall(EquipWeapon, _G.SelectWeapon or "Melee") end
                ...
                local pos = CFrame.new(0, 15, 0)
                if type(TweenTo) == "function" then
                    TweenTo(root.CFrame * pos)
                else
...
local _aimbotOk1, v1 = pcall(function()
    return assert(loadstring(game:HttpGet("https://raw.githubusercontent.com/PlockScripts/Aimbot-skill-config/refs/heads/main/Aimbot.lua")))()
end)
if not _aimbotOk1 or type(v1) ~= "table" then
    v1 = {
        SetPlayerSilentAim = function() end,
        SetNPCSilentAim = function() end,
        Pause = function() end,
    }
    warn("[GalaxyHub] Aimbot module 1 không tải được.")
end
...
local _aimbotOk2, v1 = pcall(function()
    return assert(loadstring(game:HttpGet("https://raw.githubusercontent.com/PlockScripts/Aimbot-skill-config/refs/heads/main/Aimbot.lua")))()
end)
if not _aimbotOk2 or type(v1) ~= "table" then
    v1 = {
        SetPlayerSilentAim = function() end,
        SetNPCSilentAim = function() end,
        Pause = function() end,
    }
    warn("[GalaxyHub] Aimbot module 2 không tải được.")
end
...
elseif game.PlaceId == 7449423635 or game.PlaceId == 100117331123089 then
	...
	SEA = 1
	warn("[GalaxyHub] PlaceId chưa nằm trong danh sách Sea; tiếp tục với SEA=1 để UI vẫn hiện.")
end
